...
InitializeConfig() {
  ...
  Xincgc := GetConfigKey("ServerArguments", "Xincgc", "1")
  ...
  AutomateBackups := GetConfigKey("Backups", "AutomateBackups", "1")
  ...
  DebugMode := GetConfigKey("Debugging", "DebugMode", "0")
}
...
;Returns all worlds in the config, one per line
ReadWorlds() {
...
;Deletes all worlds in the config file.  This is used once before writing new ones.
DeleteWorlds() {
  Global GUIPATH
...
;Writes the world list to the config file.
WriteWorlds(Worlds) {
  Global GUIPATH
...
;Returns the contents of server.properties
ReadServerProps() {
  Global MCServerPath
...
;Puts data into server.properties
...
;This retrieves the server log line by line, picking up where last left off, and sends it to the parser
GetLog() {
  Global MCServerPath
  Global LogFilePointer
...
ParseLogIntake(ByRef Line) {
  ...
  If (RegExMatch(Line, "i)\[INFO].*(Kicking|Kicked)")) {
    SendServer("list")
  }
